$(document).ready(function () {
  $("#saveform").submit(function (e) {
    ...
    if (errores != "") {
      ...
      $.ajax({
        type: "POST",
        url: "./Controllers/clientesControllers.php?action=save",
        data: parametros,
        cache: false,
        contentType: false,
        processData: false,
        ...
        success: function (datos) {
          if (datos.trim() == "exito") {
            ...
            $(".mensajedetexto").html(
              '<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>ERROR...!'+datos+' </strong><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
            );
          }
          cargar();
  ...
  $("#editform").submit(function (e) {
    ...
    if (errores != "") {
      ...
      $.ajax({
        type: "POST",
        url: "./Controllers/ClientesControllers.php?action=edit",
        data: parametros,
        cache: false,
        contentType: false,
        processData: false,
        ...
        success: function (datos) {
          if (datos.trim() == "exito") {
            ...
            $(".mensajedetextoedit").html(
              '<div class="alert alert-danger alert-dismissible fade show" role="alert"><strong>'+datos+' </strong><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'
            );
          }
          cargar();
          $(":submit").attr("disabled", false);
...
function eliminar(id) {
  if (confirm("Seguro de Eliminar este cliente")) {
    $.ajax({
      type: "POST",
      url: "./Controllers/ClientesControllers.php?action=eliminar&id=" + id,
